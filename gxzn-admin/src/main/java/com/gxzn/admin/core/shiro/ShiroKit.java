...
public class ShiroKit {
    ...
    /**
     * 验证当前用户是否拥有指定权限Code,使用时与hasPermissionCode 搭配使用
     *
     * @param code 权限Code
     * @return 拥有权限：true，否则false
     */
    public static boolean hasPermissionCode(String code) {
        ShiroUser user = getUser();
        return code != null && code.length() > 0 && user != null && user.getResourcesCodes() != null
                && user.getResourcesCodes().contains(code);
    }
    
    /**
     * 与hasPermissionCode标签逻辑相反，当前用户没有制定权限时，验证通过。
     *
     * @param code 权限Code
     * @return 拥有权限：true，否则false
     */
    public static boolean lacksPermissionCode(String code) {
    ...
    public static List<Long> getDeptDataScope() {
        ...
        List<Long> subDeptIds = ConstantFactory.me().getSubDeptId(Convert.toStr(deptId));
    ...
    public static boolean isAdmin() {
        ...
        for (Long integer : roleList) {
            String singleRoleTip = ConstantFactory.me().getSingleRoleTip(Convert.toStr(integer));
    ...
    public static ShiroUser createShiroUser(User user) {
        ...
        shiroUser.setDeptName(ConstantFactory.me().getDeptName(Convert.toStr(user.getDeptId())));
